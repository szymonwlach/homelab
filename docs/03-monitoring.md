...
Remove Prometheus and there's nothing to draw — it's the memory of the system.
node-exporter only knows _now_.

## Why pull, not push

Prometheus goes out and fetches metrics. Targets don't send anything.

The payoff: if a host dies, Prometheus knows immediately — the target simply
stops responding and shows up as DOWN. In a push model, silence is ambiguous:
did the agent crash, or did it just have nothing to send?
...
This is the same idea Kubernetes calls _service discovery_.

Note the two different worlds:

- **container → container** (same Docker network): service name
- **browser → container** (from outside): host IP + published port
...
- `rslave` — mount propagation flows host → container only. New disks mounted on
  the host become visible inside; nothing the container mounts leaks back out.
...
So DNS was suspect. Then, same container, different tool:
...
history — deleting it in a later commit does not remove it. That's the whole
